...
class LeftPaddleMovement : public garnish::System {
    void update(garnish::ECSController& world) override {
        auto leftPaddles = world.get_entities<LeftPaddle, garnish::RigidBody>();

        if (leftPaddles.size() > 1) std::cout << "ERROR, more then one left paddle" << std::endl;
...
class RightPaddleMovement : public garnish::System {
    void update(garnish::ECSController& world) override {
        auto rightPaddles = world.get_entities<RightPaddle, garnish::RigidBody>();

        if (rightPaddles.size() > 1) std::cout << "ERROR, more then one right paddle" << std::endl;
...
int main() {
    ...
    app.get_controller().register_system<LeftPaddleMovement>(0);
    app.get_controller().register_system<RightPaddleMovement>(0);
    ...
    auto ball = app.get_controller().create_entity_with_components(
        ...
        garnish::RigidBody{ .velocity = glm::vec3{ 2.0f, 0.0f, 0.0f }, .acceleration = glm::vec3{ 0.0f }, .inv_mass = 100.0f, .dampening = 1.0f },
